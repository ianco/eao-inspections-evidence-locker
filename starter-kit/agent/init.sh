...
select example in "1" "2" "3"; do
    case $example in
        ...
        2 ) 
            ...
            sed -i.bak "s/ INDY_GENESIS_URL/ #INDY_GENESIS_URL/" eao-agent/config/settings.yml
            # sed -i.bak "s/ AUTO_REGISTER_DID/ #AUTO_REGISTER_DID/" eao-agent/config/settings.yml
            find docker eao-agent -name "*.bak" -type f|xargs rm -f
        ...
        3 ) 
            ...
            echo NOTE: TheOrgBook API and Application URLs must be updated in eao-agent/config/settings.yml
...
sed -i.bak "s/my-organization_0000000000000000/${MY_SEED}/g" eao-agent/config/settings.yml
sed -i.bak "s#TOBAPIURL#${__TOBAPIURL}#g" eao-agent/config/settings.yml
sed -i.bak "s#TOBAPPURL#${__TOBAPPURL}#g" eao-agent/config/settings.yml
sed -i.bak "s#GENESISURL#${GENESIS_URL}#g" eao-agent/config/settings.yml
find eao-agent/config -name "*.yml" -exec sed -i.bak "s/my-org-full-name/${ORG_TITLE}/g" {} +
find eao-agent/config -name "*.yml" -exec sed -i.bak s/my-organization/${MY_ORG}/g {} +
find eao-agent/config -name "*.yml" -exec sed -i.bak s/my-permit/${MY_PERMIT}/g {} +
find eao-agent -name "*.bak" -type f|xargs rm -f
...
# find eao-agent/config -name "*.yml" -exec sed -i.bak s/X3tCbZSE9uUb223KYDWd6o/$MY_DID/g {} +
# find eao-agent -name "*.bak" -type f|xargs rm -f
...
grep -E "${ORG_TITLE}|${MY_ORG}|${MY_PERMIT}|${MY_SEED}|${__TOBAPIURL}|${__TOBAPPURL}|${GENESIS_URL}" eao-agent/config/*.yml
# grep ${ORG_TITLE} eao-agent/config/*.yml
# grep ${MY_ORG} eao-agent/config/*.yml
# grep ${MY_PERMIT} eao-agent/config/*.yml
# grep ${MY_DID} eao-agent/config/*.yml
# grep ${MY_SEED} eao-agent/config/*.yml
# grep ${__TOBAPIURL} eao-agent/config/*.yml
# grep ${__TOBAPPURL} eao-agent/config/*.yml
# grep ${GENESIS_URL} eao-agent/config/*.yml
